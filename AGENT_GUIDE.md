...
### Q4：會自動建立程式需要的 Google 試算表嗎？還是需要我自己手動建？
* **解法**：這要看你是如何開始你的專案的：
  1. **連結現有專案 (`clone`)**：如果原本雲端就有專案了，`clasp` 只會把代碼下載下來，不會建立新的試算表。
  2. **建立全新專案 (`create`)**：如果你使用 `create` 指令並指定類型為 `sheets`：
     ```bash
     npx @google/clasp create --title "專案名稱" --type sheets
     ```
